...
const SortableItem: React.FC<{
  ...
  return (
    <Box ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <Typography>{name} ({roll})</Typography>
      <Box>
...
const GMView: React.FC = () => {
  ...
  const current = useSelector((state: RootState) => state.room.currentTurnIndex);
  ...
  return (
    <Box p={2}>
      <Typography variant="h6" mb={2}>GM Controls</Typography>
      ...
      <Box display="flex" flexDirection="column" gap={2} mb={4}>
        ...
        <FormControlLabel
          control={<Checkbox checked={monsterHidden} onChange={e => setMonsterHidden(e.target.checked)} />}
          label="Hidden"
        />
        <Button
          variant="contained"
